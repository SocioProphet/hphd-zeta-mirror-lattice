"""Tests for Prime Harness v0.2 M2b π N-scan."""
...
def test_model1_pi_scan_report_shape(tmp_path) -> None:
    ...
    assert report.schema_version == "prime-harness-v0.2-m2b-pi-scan"
    assert report.interval_name == "tiny"
    assert report.manifest_hash
    assert len(report.points) == 2
    assert [point.n for point in report.points] == [1, 2]
    assert len(report.saturation_envelope) == 2
    envelope_values = [value for _, value in report.saturation_envelope]
    assert envelope_values == sorted(envelope_values)
    assert report.branch_convention == "N1: real-u integral primary; complex-Ei not used"
    assert report.result_status == "measurement_only_no_novelty_claim"


def test_model1_pi_scan_report_json_roundtrip(tmp_path) -> None:
    primary = tmp_path / "odlyzko_fixture.txt"
    independent = tmp_path / "lmfdb_fixture.txt"
    fixture = _fixture_zero_lines()
    primary.write_text(fixture, encoding="utf-8")
    ...
    report = run_model1_pi_scan(
        interval=Interval("tiny", 100, 105),
        delta_u=0.05,
        primary_zero_table_path=primary,
        independent_zero_table_path=independent,
        n_values=(1,),
    ...
    write_pi_scan_report(report, output)
    payload = json.loads(output.read_text(encoding="utf-8"))

    assert payload["schema_version"] == "prime-harness-v0.2-m2b-pi-scan"
    assert payload["zero_table_provenance"]["status"] == "validated"
    assert payload["points"][0]["n"] == 1
    assert payload["branch_convention"] == "N1: real-u integral primary; complex-Ei not used"
